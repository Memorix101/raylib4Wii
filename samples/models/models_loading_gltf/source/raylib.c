...
int main(void)
{
    ...
    // Load model
    Model model = LoadModel("sd:/res/gltf/robot.glb");
    Vector3 position = { 0.0f, 0.0f, 0.0f }; // Set model world position

    // Load model animations
    int animCount = 0;
    ModelAnimation *anims = LoadModelAnimations("sd:/res/gltf/robot.glb", &animCount);
    ...
    while (!WindowShouldClose())        // Detect window close button or ESC key
    {
        ...
        if (IsGamepadButtonPressed(0, GAMEPAD_BUTTON_LEFT_FACE_RIGHT)) animIndex = (animIndex + 1)%animCount;
        else if (IsGamepadButtonPressed(0, GAMEPAD_BUTTON_LEFT_FACE_LEFT)) animIndex = (animIndex + animCount - 1)%animCount;
